...
import { GroupProjectsDashboard } from '~/api/Dashboard/api';
import { getGroupProjectsDashboard } from '~/api/Dashboard/query';
...
export const GroupProjectSection = () => {
  const { data, isLoading } = getGroupProjectsDashboard();
      ...
      <Box display="flex" gap={2}>
        ...
        <Box>
          <Typography variant="h6" color={'text.secondary'}>
            <Trans>Group projects</Trans>
          </Typography>
          {data.projects.map((item) => (
            <GroupProjectTask key={item.groupProjectID} project={item} />
          ))}
        </Box>
      </Box>
    </Paper>
...
const GroupProjectTask = ({ project }: { project: GroupProjectsDashboard }) => {
  return (
    <Box key={project.groupProjectID}>
...
const ProjectCard = ({ project }: { project: GroupProjectsDashboard }) => {
  return (
